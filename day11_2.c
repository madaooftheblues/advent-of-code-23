...
unsigned long long get_distances(struct point galaxies[], int galaxy_count, int r[], int c[])
{
  ...
  for(int i = 0; i < galaxy_count; i++)
  {
    ...
    {
      if(j <= i) continue;
      unsigned long long dist = manhatt_dist(&galaxies[i], &galaxies[j]);
      int vl, vs, hl, hs;
      ...
      for(int k = vs; k < vl;  k++)
      {
        if(r[k]) dist += GALAXY_EXPANSION - 1;
      }
      for(int k = hs; k < hl; k++)
      {
        if(c[k]) dist += GALAXY_EXPANSION - 1;
      }
      distance_sum += dist;
      pair++;
...
int main()
{
  ...
  unsigned long long distance_count;
